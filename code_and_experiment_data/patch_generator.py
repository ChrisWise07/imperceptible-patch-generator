...
from utils import (
    get_previous_steps, get_previous_training_data, record_attack_training_data, 
    save_image_from_np_array, plot_predictions
)

from main import (
    training_data_directory, final_patches_directory, 
    final_patched_images_directory, final_predictions_images_directory, args
) 

def generate_adversarial_patch(attack: RobustDPatch, image: Image_For_Patch) -> None:
    ...
    if args.previous_experiment_directory_name:
        previous_num_steps = get_previous_steps(
            args.previous_experiment_directory_name, 
            image_name
        )
    else:
...
def generate_rrap_for_image(image_name, file_type):
    ...
    image_height, image_width = (
        image.image_as_np_array.shape[1], image.image_as_np_array.shape[2]
    )
    ...
    attack = RobustDPatch(
        estimator=FRCNN, max_iter=args.max_iter, batch_size=1, verbose=False, 
        rotation_weights=(1, 1, 0, 1), brightness_range=(0.4, 1.6),
        image_to_patch=image, training_data_path=training_data_path,   
    ...
    predictions_class, predictions_boxes, predictions_score = plot_predictions(
        object_detector=FRCNN, image=image_adv_as_np_array, threshold=0.5,
